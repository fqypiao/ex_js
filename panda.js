...
var exkey_public = 'a9b99f54c1210823188a86425ed27ea06858680_t53ehxs65nxhco1dpbn'
var exkey_private = 'a9b99f54c1210823188a86425ed27ea06858680_t53ehxs65nxhco1dpbn'
...
function panda_leapover(exkey) {
    ...
    xhr.onerror = function() {
        if (confirm(panda_lang_0)) {
            panda_leapover(exkey);
        };
    };
    xhr.onreadystatechange = function() {
        if (this.readyState === 4 && this.status === 200) {
            if (!this.responseText.match(/<link(.*?)exhentai(.*?)>/)) {
                panda_exkeyset();
                return;
            };
...
function panda_loadpage(gid, token, numb, exec) {
    ...
    xhr.onerror = function() {
        if (confirm(panda_lang_0)) {
            panda_loadpage(gid, token, numb, exec);
        };
    };
    xhr.onreadystatechange = function() {
        if (this.readyState === 4 && this.status === 200) {
...
function panda_showlist() {
    ...
    if (!panda_filefrom || panda_filefrom < 0) {
        panda_filefrom = 1;
    };
    if (!panda_filefinl || panda_filefinl > panda_fileqnty) {
        panda_filefinl = panda_fileqnty;
    };
    if (panda_filefrom > panda_filefinl) {
        alert(panda_lang_8);
        return;
    };
...
function panda_showprev() {
    ...
    if (!panda_readqnty && panda_readqnty !== '') {
        return;
    };
    panda_readqnty = parseInt(panda_readqnty);
    if (!panda_readqnty || panda_readqnty < 0) {
        panda_readqnty = panda_fileqnty;
    };
...
function panda_shownext() {
    ...
    var panda_readfrom = parseInt(document.getElementById('panda_filefrom').title);
    var panda_readfinl = parseInt(document.getElementById('panda_filefinl').title);
    if (panda_readfinl == panda_fileqnty) {
        alert(panda_lang_9);
        return;
    };
    var panda_readqnty = prompt(panda_lang_7, panda_readfinl - panda_readfrom + 1);
    ...
    if (panda_filefinl > panda_fileqnty) {
        panda_filefinl = panda_fileqnty;
    };
    document.getElementById('panda_filefrom').value = panda_filefrom;
    document.getElementById('panda_filefinl').value = panda_filefinl;
...
if (!document.domain.match(/^e[x|-]hentai\.org$/)) {
    window.location.href = 'https://exhentai.org/favicon.ico';
} else if (document.getElementById('gdt') && !document.getElementById('panda_plus')) {
    panda_plusfunc();
} else if (document.getElementById('img')) {
    window.nl = function(adds) {
        panda_loadfile(gid, window.location.href.match(/https:\/\/e[x|-]hentai\.org\/s\/(\w+)\/(\d+)-(\d+)/)[3], window.location.href.match(/https:\/\/e[x|-]hentai\.org\/s\/(\w+)\/(\d+)-(\d+)/)[1], adds, function(info) {
            if (!info) {
                return;
            };
            document.getElementById('img').src = info.show;
            document.getElementById('loadfail').setAttribute('onclick', 'return nl(\'' + info.adds + '\')');
        });
    }
} else if (document.body.innerHTML.match(/^err/)) {
    if (window.location.pathname == '/fullimg.php') {
        document.body.innerHTML = '<img id="img" src="" alt="Loading..." style="max-width:100%;" />';
        panda_sniffimg(true, function() {
            var img = new Image();
...
} else if (window.location.pathname == '/favicon.ico' || !document.head.innerHTML) {
